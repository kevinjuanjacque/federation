...
export const index = () => {
  ...
  const handlerClickDownload = async (id) => {
    try {
      const resp = await axios.get(
        process.env.NEXT_PUBLIC_BACKEND_URL + '/facturas/boleta/' + id,
        {
  ...
  const handlerDelet = async (id) => {
    try {
      ...
      setBoletas(Boletas.filter((item) => item._id !== id));
    } catch (error) {
      console.log(error);
    }
  };
